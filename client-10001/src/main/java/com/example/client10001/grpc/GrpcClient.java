...
public class GrpcClient {
    ...
    public void sendSteam(String srcPartyId, String dstPartyId, String name, String url, String operator, long overallTimeout) {
        ...
        Transfer.RollSiteHeader rollSiteHeader = Transfer.RollSiteHeader.newBuilder()
                .setSrcRole("dppc").setSrcPartyId(srcPartyId).setDstRole("dppc").setDstPartyId(dstPartyId).build();
        Proxy.Metadata metadata = Proxy.Metadata.newBuilder()
                ...
                .setConf(conf)
                .setExt(ByteString.copyFrom(rollSiteHeader.toByteArray()))
                .build();
        Proxy.Packet packet1 = Proxy.Packet.newBuilder().setHeader(metadata)
                .setBody(Proxy.Data.newBuilder().setKey("test-key1")
                        .setValue(ByteString.copyFrom("test message 1".getBytes())).build()).build();

        Proxy.Packet packet2 = Proxy.Packet.newBuilder().setHeader(metadata)
                .setBody(Proxy.Data.newBuilder().setKey("test-key2")
                        .setValue(ByteString.copyFrom("test message 2".getBytes())).build()).build();
        packets.add(packet1);
        packets.add(packet2);
        for (Proxy.Packet packet : packets) {
            log.info("send stream request from client, partyId: {}, packet: {}", srcPartyId, packet);
